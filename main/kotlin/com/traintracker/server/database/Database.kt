...
object HspUnitHistory : Table("hsp_unit_history") {
    ...
    val idxDateDep = index("idx_hsp_unit_date_dep", false, serviceDate, scheduledDep)
    val idxDateUid = index("idx_hsp_unit_date_uid", false, serviceDate, uid)
}
...
object AppDatabase {
    ...
    fun getAllocationByUid(uid: String, date: String? = null): AllocationResult? = transaction {
        ...
        exec("SELECT core_id, headcode, service_date, operator, vehicles, units, unit_count FROM allocation_consists WHERE substr(core_id, length(headcode)+2, length('$safeUid')) = '$safeUid'$headcodeFilter AND service_date = '$safeDate' UNION ALL SELECT core_id, headcode, service_date, operator, vehicles, units, unit_count FROM allocation_history WHERE substr(core_id, length(headcode)+2, length('$safeUid')) = '$safeUid'$headcodeFilter AND service_date = '$safeDate' AND NOT EXISTS (SELECT 1 FROM allocation_consists WHERE substr(core_id, length(headcode)+2, length('$safeUid')) = '$safeUid'$headcodeFilter AND service_date = '$safeDate') ORDER BY service_date DESC LIMIT 1") { rs ->
            if (rs.next()) {
    ...
    fun getAllocationsByUids(uids: Map<String, String>, date: String? = null): Map<String, AllocationResult> {
        ...
        transaction {
            ...
            // Stage 3: hsp_unit_history fallback for any uid still missing
            val missingAfterStage2 = safeUids.filter { (uid, _) -> !results.containsKey(uid) }
            if (missingAfterStage2.isNotEmpty()) {
                val missingIn = missingAfterStage2.keys.joinToString(",") { "'$it'" }
                exec("SELECT uid, units, vehicles, unit_count FROM hsp_unit_history WHERE uid IN ($missingIn) AND service_date = '$safeDate'") { rs ->
                    while (rs.next()) {
                        val uidMatch = rs.getString("uid") ?: continue
                        if (results.containsKey(uidMatch)) continue
                        val allUnits    = (rs.getString("units") ?: "").split(",").filter { it.isNotEmpty() }
                        val allVehicles = (rs.getString("vehicles") ?: "").split(",").filter { it.isNotEmpty() }
                        val (units, vehicles, unitCount) = correctAllocation(allUnits, allVehicles, rs.getInt("unit_count"))
                        results[uidMatch] = AllocationResult(
                            coreId      = uidMatch,
                            headcode    = "",
                            serviceDate = safeDate,
                            operator    = "",
    ...
    fun snapshotUnitAllocations(date: String) {
        ...
        transaction {
            exec("SELECT uid, units, vehicles, unit_count FROM hsp_unit_history " +
                ...
                if (rs.next()) result1 = extractAllocationFromRs(rs, rs.getString("uid") ?: "", date)
            }
        }
        ...
        transaction {
            exec("SELECT units, vehicles, unit_count FROM allocation_history " +
                ...
                if (rs.next()) result2 = extractAllocationFromRs(rs, rid, date)
            }
        }
        ...
        if (scheduledDep.isNotEmpty() && resolvedOriginCrs.isNotEmpty()) {
            ...
            transaction {
                exec("SELECT uid, units, vehicles, unit_count FROM hsp_unit_history " +
                    ...
                    if (rs.next()) result3 = extractAllocationFromRs(rs, rs.getString("uid") ?: "", date)
                }
            }
