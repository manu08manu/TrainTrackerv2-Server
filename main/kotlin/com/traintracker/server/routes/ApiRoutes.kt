                                ...
                                when {
                                    ...
                                    !hasOwnLo && !hasOwnLt -> {
                                        ...
                                        if (hasOverlap) {
                                            ...
                                            // Divert: O rows replace P entirely within the O time window.
                                            // pBefore is trimmed to the last P stop that also appears in pAfter
                                            // (i.e. a stop on the common route), or failing that, the last P stop
                                            // before oFirst whose CRS is non-null (a real passenger station).
                                            val oFirst = bestRows.minOf { it[Schedules.scheduledTime] }
                                            val oLast = bestRows.maxOf { it[Schedules.scheduledTime] }
                                            val pAfter = pRows.filter { it[Schedules.scheduledTime] > oLast }
                                            val pAfterTiplocs = pAfter.map { it[Schedules.tiploc] }.toSet()
                                            val pBeforeRaw = pRows.filter { it[Schedules.scheduledTime] < oFirst }
                                            // Find the last pBefore stop that is also in pAfter (common route anchor)
                                            val lastCommonIdx = pBeforeRaw.indexOfLast { it[Schedules.tiploc] in pAfterTiplocs }
                                            // If no common anchor, find the last CRS stop in pBefore that has
                                            // at least one null-CRS (junction/signal) stop after it before oFirst.
                                            // This identifies the last real station before the divergence gap.
                                            val cutIdx = if (lastCommonIdx >= 0) lastCommonIdx
                                                else {
                                                    val lastNullAfter = pBeforeRaw.indexOfLast { it[Schedules.crs] == null }
                                                    if (lastNullAfter > 0)
                                                        pBeforeRaw.subList(0, lastNullAfter)
                                                            .indexOfLast { it[Schedules.crs] != null }
                                                    else
                                                        pBeforeRaw.indexOfLast { it[Schedules.crs] != null }
                                                }
                                            val pBefore = if (cutIdx >= 0) pBeforeRaw.subList(0, cutIdx + 1)
                                                else pBeforeRaw
                                            (pBefore + bestRows + pAfter).sortedBy { it[Schedules.scheduledTime] }
                                        }
                                    }
                                    hasOwnLo && !hasOwnLi -> {
