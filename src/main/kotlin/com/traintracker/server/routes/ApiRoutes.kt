...
import com.traintracker.server.kafka.darwinPlatforms
...
// ─── File-level constants and helpers ────────────────────────────────────────

/** STP schedule priority: N beats O beats P; C is cancelled/dropped. */
private val STP_PRIORITY = mapOf(
    'N' to 0, 'O' to 1, 'P' to 2, 'C' to 3
)

private fun isPassengerHeadcode(headcode: String) =
    headcode.firstOrNull()?.let { it in "1239" } == true

private fun String.toMinutes(): Int =
    split(":").let { p -> (p.getOrNull(0)?.toIntOrNull() ?: 0) * 60 + (p.getOrNull(1)?.toIntOrNull() ?: 0) }

/** Resolve platform and confirmed status in a single lookup.
 *  Returns Pair(platform, confirmed) — Darwin live → Darwin DB → CIF scheduled */
private fun resolvePlatformWithConfirmed(uid: String, tiploc: String, cifPlatform: String?): Pair<String?, Boolean> {
    val live = darwinPlatforms[uid]?.get(tiploc)
    if (live != null) return Pair(live, true)
    val db = try { com.traintracker.server.database.AppDatabase.getDarwinPlatform(uid, tiploc) } catch (_: Exception) { null }
    if (db != null) return Pair(db, true)
    return Pair(cifPlatform, false)
}

private val staticHtmlCache = java.util.concurrent.ConcurrentHashMap<String, String>()
private suspend fun io.ktor.server.application.ApplicationCall.serveStaticHtml(
    path: String, notFoundMsg: String = "Not found"
) {
    val html = staticHtmlCache.getOrPut(path) {
        val file = java.io.File(path)
        if (file.exists()) file.readText() else ""
    }
    if (html.isNotEmpty()) respondText(html, io.ktor.http.ContentType.Text.Html)
    else respond(io.ktor.http.HttpStatusCode.NotFound, notFoundMsg)
}

private suspend fun io.ktor.server.application.ApplicationCall.handleBoardRequest(arrivalsOnly: Boolean) {
    val crs = parameters["crs"]?.uppercase()?.trim()
    if (crs == null) { respond(HttpStatusCode.BadRequest, "crs required"); return }
    val window = parameters["window"]?.toIntOrNull() ?: 120
    val offset = parameters["offset"]?.toIntOrNull() ?: 0
    val (start, end) = timeWindow(window, offset)
    val boardType = if (arrivalsOnly) "ARRIVALS" else "DEPARTURES"
    val services = withContext(Dispatchers.IO) { annotateSplits(enrichWithUnits(
        queryBoard(crs, start, end, arrivalsOnly = arrivalsOnly, passengerOnly = true, excludePassing = true)
    )) }
    respond(BoardResponse(crs, boardType, start, end, services))
}

private fun org.jetbrains.exposed.sql.Transaction.headcodeForUid(uid: String): String {
    var result = ""
    try {
        exec("SELECT headcode FROM schedules WHERE uid = '$uid' AND stop_type = 'LO' AND stp_indicator != 'C' LIMIT 1") { rs ->
            if (rs.next()) result = rs.getString("headcode") ?: ""
        }
    } catch (_: Exception) {}
    return result
}

private fun AppDatabase.AllocationResult.toResponse() = AllocationResponse(
    coreId      = coreId,
    headcode    = headcode,
...
fun Application.configureRoutes() {
    routing {
        // ── GET / (main site) ────────────────────────────────────────────
        get("/")          { call.serveStaticHtml("/opt/traintracker/index.html") }

        // ── GET /dashboard ───────────────────────────────────────────────
        get("/dashboard") { call.serveStaticHtml("/opt/traintracker/dashboard.html", "Dashboard not found") }
        route("/api") {
            authRoutes()
            ...
            get("/departures") { call.handleBoardRequest(arrivalsOnly = false) }

            // ── GET /api/arrivals?crs=MAN&window=120 ──────────────────────
            get("/arrivals")   { call.handleBoardRequest(arrivalsOnly = true) }
            ...
            get("/service/{uid}") {
                ...
                val callingPoints = transaction {
                    Schedules.selectAll().where { Schedules.uid eq uid }
                        ...
                        .toList()
                            ...
                            if (atCrs.isNotEmpty()) {
                                val bestHasAnyLo = bestRows.any { it[Schedules.stopType] == "LO" }
                            val bestHasMatchingLo = bestRows.any { it[Schedules.stopType] == "LO" && it[Schedules.crs] == atCrs }
                                if (bestHasAnyLo && !bestHasMatchingLo) {
                            ...
                            if (bestSttpChar != null && bestSttpChar != 'P') {
                                ...
                                when {
                                    ...
                                    !hasOwnLo && !hasOwnLt -> {
                                        // Partial stop override or divert: O rows either replace specific tiplocs,
                                        // or define a completely different routing within a time window
                                        val overriddenTiplocs = bestRows.map { it[Schedules.tiploc] }.toSet()
                                        val pRows = rows.filter { it[Schedules.stpIndicator] == 'P' }
                                            .sortedBy { it[Schedules.scheduledTime] }
                                        val hasOverlap = pRows.any { it[Schedules.tiploc] in overriddenTiplocs }
                                        // If all O rows are pure pathing/junction points (no CRS, all passes),
                                        // this is a minor WTT timing adjustment — never a passenger diversion.
                                        // Slot O rows into P at their time position and leave P intact.
                                        val allOArePassingPoints = bestRows.all {
                                            it[Schedules.crs].isNullOrEmpty() && it[Schedules.isPass]
                                        }
                                        if (allOArePassingPoints) {
                                            val filteredP = pRows.filter { it[Schedules.tiploc] !in overriddenTiplocs }
                                            (bestRows + filteredP).sortedBy { it[Schedules.scheduledTime] }
                                        } else if (hasOverlap) {
                                            // Replacement: O rows substitute specific P tiplocs
                                            val filteredP = pRows.filter { it[Schedules.tiploc] !in overriddenTiplocs }
                                            (bestRows + filteredP).sortedBy { it[Schedules.scheduledTime] }
                                        } else {
                                            // Divert: O rows replace P within the diverted section.
                                            // Split P by index, not time — O times may overlap P times
                                            // (e.g. divert starts at 15:20 but P origin is at 15:48).
                                            // Strategy: find the first P tiploc that reappears after the divert
                                            // (pAfter anchor), split P into before/after that index.
                                            val oLast = bestRows.maxOf { it[Schedules.scheduledTime] }
                                            val pAfterByTime = pRows.filter { it[Schedules.scheduledTime] > oLast }
                                            val pAfterTiplocs = pAfterByTime.map { it[Schedules.tiploc] }.toSet()
                                            // Find the earliest index in pRows where a pAfter tiploc appears
                                            val firstPAfterIdx = pRows.indexOfFirst { it[Schedules.tiploc] in pAfterTiplocs }
                                            val pBeforeCandidate = if (firstPAfterIdx > 0) pRows.subList(0, firstPAfterIdx) else emptyList()
                                            // If pBefore contains actual intermediate station calls (LI with CRS),
                                            // the O divert has replaced that entire section — drop pBefore entirely.
                                            // If it only contains the P origin (LO) and/or CRS-less passing points,
                                            // the origin is still valid (e.g. train departs WAT then takes amended path).
                                            val pBeforeHasStations = pBeforeCandidate.any {
                                                it[Schedules.stopType] == "LI" && !it[Schedules.crs].isNullOrEmpty()
                                            }
                                            // If pBefore has intermediate stations replaced by the divert,
                                            // still keep the LO (origin) — O with no LO inherits P origin
                                            val pBefore = if (pBeforeHasStations)
                                                pBeforeCandidate.filter { it[Schedules.stopType] == "LO" }
                                            else pBeforeCandidate
                                            val pAfter = if (firstPAfterIdx >= 0) pRows.subList(firstPAfterIdx, pRows.size)
                                                         else pAfterByTime
                                            // Preserve structural order: pBefore → O divert → pAfter
                                            // Do NOT sort by time — O times overlap P times in reroutes
                                            // Deduplicate by CRS: if O row already covers a CRS, drop P row with same CRS
                                            val oCrsCovered = bestRows.mapNotNull { it[Schedules.crs].takeIf { c -> !c.isNullOrEmpty() } }.toSet()
                                            val pBeforeDeduped = pBefore.filter { it[Schedules.crs].isNullOrEmpty() || it[Schedules.crs] !in oCrsCovered }
                                            val pAfterDeduped  = pAfter.filter  { it[Schedules.crs].isNullOrEmpty() || it[Schedules.crs] !in oCrsCovered }
                                            (pBeforeDeduped + bestRows + pAfterDeduped)
                                        }
                                    }
                                    hasOwnLo && !hasOwnLi -> {
                        ...
                        .also { rows ->
                            ...
                            if (safeHc.isNotEmpty()) {
                                // TRUST CRS → CIF CRS aliases for Elizabeth line platform codes
                                val trustCrsAlias = mapOf(
                                    "PDX" to "PAD", "FDX" to "FOG", "LSX" to "LST",
                                    "ABX" to "ABW", "WHX" to "WWC"
                                )
                                exec("SELECT crs, stanox, scheduled_time, actual_time, is_cancelled, cancel_reason FROM trust_movements WHERE uid='$uid' AND event_ts >= datetime('now', '-12 hours') ORDER BY event_ts DESC") { rs ->
                                    while (rs.next()) {
                                        ...
                                        val rawCrs = rs.getString("crs")?.takeIf { it.isNotEmpty() }
                                            ?: CorpusLookup.crsFromStanox(rs.getString("stanox") ?: "")
                                        val crs = rawCrs?.let { trustCrsAlias[it] ?: it }
                        ...
                        .map { row ->
                            ...
                            CallingPointResponse(
                                ...
                                platform          = resolvePlatformWithConfirmed(uid, tiploc, row[Schedules.platform]).first,
                                platformConfirmed = resolvePlatformWithConfirmed(uid, tiploc, row[Schedules.platform]).second,
                        ...
                        .let { rawCps ->
                            ...
                            propagatedCps2.map { cp ->
                                ...
                                if (isTrustConfirmed) {
                                    ...
                                    val sch = cp.scheduledTime.toMinutes()
                                    val act = cp.actualTime.toMinutes()
                                    var diff = act - sch
                                    ...
                                    if (diff == 0 && lastDelayMins > 0 && cp.crs == null) {
                                        val projMins = (sch + lastDelayMins + 1440) % 1440
                                    ...
                                    val projMins = (cp.scheduledTime.toMinutes() + lastDelayMins + 1440) % 1440
                                    cp.copy(actualTime = "%02d:%02d".format(projMins / 60, projMins % 60),
                                            cancelReason = lastReasonCode,
                                            isPredicted = true)
                                } else cp
                ...
                val annotatedCps = run {
                    ...
                    if (!serviceAssocs.isNullOrEmpty()) {
                        ...
                        if (mainCoaches > 0) {
                            ...
                            } else finalCps.map { it.copy(length = mainCoaches) }
                        } else finalCps
                    } else finalCps
                }
                ...
                val allCpsForStatus = previous + subsequent
                val isPartiallyCancelled = allCpsForStatus.any { it.isCancelled } && allCpsForStatus.any { !it.isCancelled }
                val topCancelReason = allCpsForStatus.firstOrNull { it.isCancelled && it.cancelReason != null }?.cancelReason
                call.respond(ServiceDetailResponse(uid, atCrs, previous, subsequent, serviceType, isPartiallyCancelled, topCancelReason))
            }
            ...
            get("/allservices") {
                ...
                val (start, end) = timeWindow(window, offset)
                val services = withContext(Dispatchers.IO) { enrichWithUnits(queryBoard(crs, start, end, arrivalsOnly = false, passengerOnly = false, excludePassing = false)) }
            ...
            get("/trust/{headcode}") {
                ...
                if (loc != null) {
                    ...
                    val dbLoc = withContext(Dispatchers.IO) { transaction {
            ...
            get("/headcode/{headcode}") {
                ...
                val services = withContext(Dispatchers.IO) { annotateSplits(enrichWithUnits(queryByHeadcode(safeHc))) }
                if (services.isEmpty()) {
            ...
            get("/unit/{unit}") {
                ...
                val services = withContext(Dispatchers.IO) { queryByUnit(safeUnit) }
                if (services.isEmpty()) {
            ...
            get("/allocation/{param}") {
                ...
                when {
                    ...
                    results.size == 1 ->
                        call.respond(results.first().toResponse())
                    else ->
            ...
            // ── GET /api/admin/crs-lookup?crs=MAN ────────────────────────────────
            get("/admin/crs-lookup") {
                if (!call.checkAuth()) return@get
                val crs = call.parameters["crs"]?.uppercase()?.trim() ?: ""
                val tiplocs = CorpusLookup.tiplocFromCrs(crs)
                call.respond(buildJsonObject {
                    put("crs", crs)
                    put("tiplocs", kotlinx.serialization.json.Json.encodeToJsonElement(tiplocs))
                })
            }
            
            // ── POST /api/admin/tiploc-crs?tiploc=STFORDI&crs=SFA ───────────────────
            post("/admin/tiploc-crs") {
                if (!call.checkAuth()) return@post
                val tiploc = call.parameters["tiploc"]?.uppercase()?.trim() ?: ""
                val crs    = call.parameters["crs"]?.uppercase()?.trim() ?: ""
                if (tiploc.isEmpty() || crs.isEmpty()) {
                    call.respond(HttpStatusCode.BadRequest, mapOf("error" to "tiploc and crs required"))
                    return@post
                }
                val oldCrs = CorpusLookup.crsFromTiploc(tiploc) ?: "(none)"
                CorpusLookup.updateTiplocCrs(tiploc, crs, "admin")
                call.respond(buildJsonObject {
                    put("tiploc", tiploc)
                    put("crs", crs)
                    put("oldCrs", oldCrs)
...
private fun queryByHeadcode(headcode: String): List<ServiceResponse> {
    return transaction {
        ...
        // ── Batch pre-fetch: eliminate N+1 per-assoc DB queries ──────────────────────
        val allAssocUids = (assocMap.values.flatten().map { it.assocUid } +
                           reverseAssocMap.values.flatten().map { it.mainUid }).toSet()
        val assocHeadcodes     = mutableMapOf<String, String>()
        val assocEffectiveLos  = mutableMapOf<String, String>()
        val assocOriginTiplocs = mutableMapOf<String, String>()
        val assocDestTiplocs   = mutableMapOf<String, String>()
        val assocAllStops      = mutableMapOf<String, MutableSet<String>>()
        val svcTermini         = mutableMapOf<String, String>()
        if (allAssocUids.isNotEmpty()) {
            val inClause = allAssocUids.joinToString(",") { "'$it'" }
            exec("SELECT uid, headcode, tiploc, stop_type, stp_indicator FROM schedules WHERE uid IN ($inClause) AND stp_indicator != 'C' ORDER BY CASE stp_indicator WHEN 'N' THEN 0 WHEN 'O' THEN 1 WHEN 'P' THEN 2 ELSE 3 END") { rs ->
                while (rs.next()) {
                    val rUid    = rs.getString("uid")       ?: continue
                    val rTiploc = rs.getString("tiploc")    ?: continue
                    val rStop   = rs.getString("stop_type") ?: continue
                    val rHc     = rs.getString("headcode")  ?: ""
                    assocAllStops.getOrPut(rUid) { mutableSetOf() }.add(rTiploc)
                    if (rStop == "LO") {
                        assocHeadcodes.putIfAbsent(rUid, rHc)
                        assocEffectiveLos.putIfAbsent(rUid, rTiploc)
                        assocOriginTiplocs.putIfAbsent(rUid, rTiploc)
                    }
                    if (rStop == "LT") assocDestTiplocs.putIfAbsent(rUid, rTiploc)
                }
            }
        }
        if (uids.isNotEmpty()) {
            val inClause = uids.joinToString(",") { "'$it'" }
            exec("SELECT uid, tiploc FROM schedules WHERE uid IN ($inClause) AND stop_type='LT' AND stp_indicator!='C'") { rs ->
                while (rs.next()) {
                    val rUid    = rs.getString("uid")    ?: continue
                    val rTiploc = rs.getString("tiploc") ?: continue
                    svcTermini[rUid] = rTiploc
                }
            }
        }
        bestRows.sortedBy { it[Schedules.scheduledTime] }.map { row ->
            ...
            val hasAnyActualTime = trustMap.entries.any { (k, v) -> k.startsWith("$uid|") && v.first.isNotEmpty() }
            var svc = ServiceResponse(
                uid           = row[Schedules.uid],
                headcode      = row[Schedules.headcode],
                atocCode      = row[Schedules.atocCode],
                scheduledTime = scht,
                platform          = resolvePlatformWithConfirmed(row[Schedules.uid], row[Schedules.tiploc], row[Schedules.platform]).first,
                platformConfirmed = resolvePlatformWithConfirmed(row[Schedules.uid], row[Schedules.tiploc], row[Schedules.platform]).second,
                isPass        = row[Schedules.isPass],
                stopType      = row[Schedules.stopType],
                ...
                actualTime    = trustEntry?.first ?: "",
                isCancelled   = if (hasAnyActualTime) false else (trustEntry?.second ?: false),
                cancelReason  = trustEntry?.third,
            ...
            if (!assocs.isNullOrEmpty()) {
                for (assoc in assocs) {
                    val assocHeadcode = assocHeadcodes[assoc.assocUid] ?: ""
                    ...
                    if (assoc.assocType in listOf("VV", "JJ")) {
                        val isPassengerSplit = isPassengerHeadcode(assocHeadcode)
                        val assocTiplocIsTerminus = svcTermini[uid] == assoc.assocTiploc
                        val assocEffectiveLo = assocEffectiveLos[assoc.assocUid] ?: ""
                        if (isPassengerSplit && !assocTiplocIsTerminus &&
                            (assocEffectiveLo == assoc.assocTiploc || assocEffectiveLo.isEmpty())) {
                            val splitDestName: String? = assocDestTiplocs[assoc.assocUid]?.let { CorpusLookup.nameFromTiploc(it) }
                            svc = svc.copy(
            ...
            if (reverseAssocs != null && reverseAssocs.isNotEmpty() && svc.couplingTiploc.isNullOrEmpty()) {
                for (assoc in reverseAssocs) {
                    if (assoc.assocType in listOf("VV", "JJ")) {
                        val parentHeadcode = assocHeadcodes[assoc.mainUid] ?: ""
                        val tiplocCrs = CorpusLookup.crsFromTiploc(assoc.assocTiploc) ?: crsfromTiplocFallback(assoc.assocTiploc) ?: assoc.assocTiploc
                        val mainCallsAtAssocTiploc = assocAllStops[assoc.mainUid]?.contains(assoc.assocTiploc) ?: false
                        if (parentHeadcode.isNotEmpty() && mainCallsAtAssocTiploc) {
                            svc = svc.copy(
    ...
    return transaction {
            ...
            exec(
                ...
                while (rs.next()) {
                    ...
                    rows.add(ServiceResponse(
                        ...
                        platform          = resolvePlatformWithConfirmed(loRow[Schedules.uid], loRow[Schedules.tiploc], loRow[Schedules.platform]).first,
                        platformConfirmed = resolvePlatformWithConfirmed(loRow[Schedules.uid], loRow[Schedules.tiploc], loRow[Schedules.platform]).second,
                    ...
                    exec(
                        ...
                        while (rs.next()) {
                            ...
                            extraRows.add(ServiceResponse(
                                ...
                                platform          = resolvePlatformWithConfirmed(loRow2[Schedules.uid], loRow2[Schedules.tiploc], loRow2[Schedules.platform]).first,
                                platformConfirmed = resolvePlatformWithConfirmed(loRow2[Schedules.uid], loRow2[Schedules.tiploc], loRow2[Schedules.platform]).second,
        ...
        sorted.map { svc ->
            ...
            val splitPortion = sorted.filter { other ->
                ...
                other.scheduledTime.toMinutes() - svc.scheduledTime.toMinutes() <= 90 &&
...
private fun annotateSplits(services: List<ServiceResponse>): List<ServiceResponse> {
    ...
    return transaction {
        ...
        if (uids.isNotEmpty()) {
            ...
            // O-schedule early terminations override P terminus
            exec("SELECT uid, tiploc FROM schedules WHERE uid IN ($inClause) AND stop_type='LT' AND stp_indicator='O'") { rs ->
                while (rs.next()) {
                    val rUid    = rs.getString("uid")    ?: continue
                    val rTiploc = rs.getString("tiploc") ?: continue
                    svcTermini[rUid] = rTiploc
                    ...
                    when (assoc.assocType) {
                        "VV" -> { // divide — main continues, assocUid splits off at assocTiploc
                            ...
                            if ((assocEffectiveLo == assoc.assocTiploc || assocEffectiveLo.isEmpty()) && isPassengerSplit) {
                                ...
                                splitToDestName2 = assocDestTiplocs[assoc.assocUid]?.let { CorpusLookup.nameFromTiploc(it) }
                            }
                        }
                        "JJ", "NP" -> { // join — assocUid joins main at assocTiploc
                            val assocOriginTiploc = assocOriginTiplocs[assoc.assocUid] ?: ""
                            val assocTiplocIsTerminus = svcTermini[svc.uid] == assoc.assocTiploc
                            val assocCallsAtJoinTiploc = assocAllStops[assoc.assocUid]?.contains(assoc.assocTiploc) ?: false
                            if (assocOriginTiploc.isNotEmpty() && assocOriginTiploc != assoc.assocTiploc && !assocTiplocIsTerminus && assocCallsAtJoinTiploc) {
...
private fun enrichWithUnits(services: List<ServiceResponse>): List<ServiceResponse> {
    val today = java.time.LocalDate.now().toString()
    val uidHeadcodes = services.filter { it.units.isEmpty() }.associate { it.uid to it.headcode }
    if (uidHeadcodes.isEmpty()) return services
    val allocMap = AppDatabase.getAllocationsByUids(uidHeadcodes, today)
    return services.map { svc ->
        if (svc.units.isNotEmpty()) return@map svc
        val alloc = allocMap[svc.uid]
...
private fun queryBoard(
    ...
    return transaction {
        val needed = listOf(
            ...
            Schedules.tiploc, Schedules.originTiploc, Schedules.destTiploc,
        ...
        .values
        .map { rows ->
            // If a UID has multiple P schedule variants (different schedule_start_date)
            // AND an O schedule, pick the P whose LO time is <= O's first row time.
            // This ensures we pair the O amendment with the correct P base schedule.
            val pVariants = rows.filter { it[Schedules.stpIndicator] == 'P' }
                .groupBy { it[Schedules.scheduleStartDate] }
            if (pVariants.size <= 1) return@map rows
            val oRows = rows.filter { it[Schedules.stpIndicator] == 'O' }
            if (oRows.isEmpty()) return@map rows
            val oFirstTime = oRows.minOf { it[Schedules.scheduledTime] }
            // Pick the P whose LO time is <= oFirstTime (closest before divert starts)
            val bestStartDate = pVariants.entries
                .mapNotNull { (startDate, pRows) ->
                    val loTime = pRows.firstOrNull { it[Schedules.stopType] == "LO" }
                        ?.get(Schedules.scheduledTime) ?: return@mapNotNull null
                    startDate to loTime
                }
                .filter { (_, loTime) -> loTime <= oFirstTime }
                .maxByOrNull { (_, loTime) -> loTime }
                ?.first
                ?: pVariants.keys.minOrNull() // fallback: earliest start date
            val nonPRows = rows.filter { it[Schedules.stpIndicator] != 'P' }
            val bestPRows = pVariants[bestStartDate] ?: rows.filter { it[Schedules.stpIndicator] == 'P' }
            bestPRows + nonPRows
        }
        .let { groups ->
            ...
            val uidsCancelled = mutableSetOf<String>()
            if (pUids.isNotEmpty()) {
                val inClause = pUids.joinToString(",") { "'$it'" }
                exec("SELECT DISTINCT uid, stp_indicator FROM schedules WHERE uid IN ($inClause) AND stp_indicator IN ('O','N','C')") { rs ->
                    while (rs.next()) {
                        val uid = rs.getString("uid") ?: continue
                        when (rs.getString("stp_indicator")) {
                            "O", "N" -> uidsWithOverride.add(uid)
                            "C"      -> uidsCancelled.add(uid)
                        }
                    }
                }
            }
            ...
            groups.forEach { rows ->
                ...
                if (best[Schedules.stpIndicator] == 'C') {
                    // Cancelled STP — drop entirely
                } else if (best[Schedules.stpIndicator] == 'P' && best[Schedules.uid] in uidsCancelled) {
                    // P service cancelled by a C entry — drop entirely
                } else if (best[Schedules.stpIndicator] == 'P' && best[Schedules.uid] in uidsWithOverride) {
            ...
            // Amended-WTT suppression:
            // (A) O schedule moves terminus before board CRS → service no longer reaches here
            // (B) O schedule moves origin after board CRS → service doesn't call here yet
            // Both are detected by comparing the O LT/LO time against the P row time at board CRS.
            if (validUids.isNotEmpty()) {
                val inClause = validUids.map { "'$it'" }.joinToString(",")
                val safeCrsVal = crs.filter { it.isLetterOrDigit() }
                // Get the P scheduled_time at board CRS for each candidate UID
                val pTimeAtCrs = mutableMapOf<String, String>()
                exec("SELECT uid, scheduled_time FROM schedules WHERE uid IN ($inClause) AND crs='$safeCrsVal' AND stp_indicator='P'") { rs ->
                    while (rs.next()) {
                        val uid = rs.getString("uid") ?: continue
                        val t   = rs.getString("scheduled_time") ?: continue
                        pTimeAtCrs[uid] = t
                    }
                }
                // Get O LT (early termination) and O LO (late origin) times
                val oLtTime = mutableMapOf<String, String>()
                val oLoTime = mutableMapOf<String, String>()
                exec("SELECT uid, stop_type, scheduled_time FROM schedules WHERE uid IN ($inClause) AND stp_indicator='O' AND stop_type IN ('LT','LO')") { rs ->
                    while (rs.next()) {
                        val uid = rs.getString("uid") ?: continue
                        val t   = rs.getString("scheduled_time") ?: continue
                ...
                for (uid in validUids) {
                    val pTime = pTimeAtCrs[uid] ?: continue
                    // (A) Early termination: O LT is at or before the P time at board CRS
                    val oLt = oLtTime[uid]
                    if (oLt != null && oLt <= pTime) { toRemove.add(uid); continue }
                    // (B) Late origin: O LO is strictly after the P time at board CRS
                    // (equal means the O origin IS the board CRS — should still show)
                    val oLo = oLoTime[uid]
                    if (oLo != null && oLo > pTime) { toRemove.add(uid); continue }
                }
                validUids -= toRemove
            }
            ...
            val pFallbackRows = if (pFallbackUids.isNotEmpty()) {
                ...
                // Also exclude UIDs where the O amendment means the service no longer
                // reaches this CRS at all, even though P had a row here:
                // (A) O LT (early termination) is at or before the P time at board CRS
                // (B) O rows are all after the P time at board CRS (late-start diversion)
                val overrideNoLongerCallsHere = mutableSetOf<String>()
                val candidatesForTimeCheck = (pFallbackUids - overrideCallsHere - overrideMovedOrigin)
                if (candidatesForTimeCheck.isNotEmpty()) {
                    val inClause3 = candidatesForTimeCheck.joinToString(",") { "'$it'" }
                    // P time at board CRS
                    val pTimeFb = mutableMapOf<String, String>()
                    exec("SELECT uid, scheduled_time FROM schedules WHERE uid IN ($inClause3) AND crs='$safeCrsVal' AND stp_indicator='P'") { rs ->
                        while (rs.next()) {
                            val uid = rs.getString("uid") ?: continue
                            val t   = rs.getString("scheduled_time") ?: continue
                            pTimeFb[uid] = t
                    ...
                    // O LT and earliest O time
                    val oLtFb    = mutableMapOf<String, String>()
                    val oFirstFb = mutableMapOf<String, String>()
                    exec("SELECT uid, stop_type, scheduled_time FROM schedules WHERE uid IN ($inClause3) AND stp_indicator='O'") { rs ->
                        while (rs.next()) {
                            val uid = rs.getString("uid") ?: continue
                            val t   = rs.getString("scheduled_time") ?: continue
                            if (rs.getString("stop_type") == "LT") oLtFb[uid] = t
                            if (oFirstFb[uid] == null || t < oFirstFb[uid]!!) oFirstFb[uid] = t
                        }
                    }
                    for (uid in candidatesForTimeCheck) {
                        val pTime = pTimeFb[uid] ?: continue
                        // (A) O terminates before board CRS
                        val oLt = oLtFb[uid]
                        if (oLt != null && oLt <= pTime) { overrideNoLongerCallsHere.add(uid); continue }
                        // (B) All O rows start after board CRS P time → late-start diversion bypasses this CRS
                        val oFirst = oFirstFb[uid]
                        if (oFirst != null && oFirst > pTime) { overrideNoLongerCallsHere.add(uid); continue }

                    }
                }
                val genuineFallbacks = pFallbackUids - overrideCallsHere - overrideMovedOrigin - overrideNoLongerCallsHere
            ...
            val combinedRows = (winnerRows.filter { it[Schedules.uid] in validUids } + pFallbackRows)
                .sortedBy { it[Schedules.scheduledTime] }
            // Suppress P-coded LI rows where an N-coded service originates (LO) at the same
            // CRS within ±1 minute — handles headcode changes where replacement is a new UID
            val safeCrsForN = crs.filter { it.isLetterOrDigit() }
            // Keys for N-coded LO replacements: "CRS|HH:MM" (±1 min)
            val nLoTimes = mutableSetOf<String>()
            exec("SELECT scheduled_time FROM schedules WHERE stp_indicator='N' AND stop_type='LO' AND crs='$safeCrsForN' AND scheduled_time >= '$windowStart' AND scheduled_time <= '$windowEnd'") { rs ->
                while (rs.next()) {
                    val t = rs.getString("scheduled_time") ?: return@exec
                    val tMins = t.replace(":", "").toIntOrNull() ?: return@exec
                    val hh = tMins / 100; val mm = tMins % 100
                    val base = hh * 60 + mm
                    for (delta in -1..1) {
                        val adj = (base + delta + 1440) % 1440
                        nLoTimes.add("$safeCrsForN|%02d:%02d".format(adj / 60, adj % 60))
                    }
                }
            }
            combinedRows.filter { row ->
                if (row[Schedules.stpIndicator] != 'P' || row[Schedules.stopType] != "LI") return@filter true
                val timeKey = "$safeCrsForN|${row[Schedules.scheduledTime]}"
                timeKey !in nLoTimes
            }.sortedBy { it[Schedules.scheduledTime] }
        }
        .sortedBy { it[Schedules.scheduledTime] }
        .let { sortedRows ->
            ...
            // XR (Elizabeth line) and LT (LU shared sections) excluded — TfL feed is sole source.
            val uidInClause = sortedRows
                .map { "'${it[Schedules.uid]}'" }.toSet().joinToString(",")
            if (uidInClause.isNotEmpty()) {
                try {
            ...
            if (uidInClause.isNotEmpty()) {
                try {
                    exec("SELECT uid, tiploc, crs, stop_type FROM schedules WHERE uid IN ($uidInClause) AND stp_indicator='O' AND stop_type IN ('LT','LO')") { rs ->
                        while (rs.next()) {
                            val uid = rs.getString("uid") ?: continue
            ...
            sortedRows.map { row ->
                ...
                val trustEntry         = stopEntry
                val cancelEntry        = stopEntry ?: if (uidEntry?.second == true) uidEntry else null
                val actualTime         = trustEntry?.first?.takeIf { it.isNotEmpty() } ?: ""
                ...
                ServiceResponse(
                    ...
                    actualTime    = actualTime,
                    isCancelled   = (cancelEntry?.second ?: false) || row[Schedules.stpIndicator] == 'C',
                    cancelReason  = cancelEntry?.third,
